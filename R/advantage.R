advantage_pars <- function(pars,SV,nr,pname="v")  {
  ...
  for (i in 1:na) {
    for (j in 1:nr) {
      ...
      d[i,j,] <- apply(q[c(notj[i],j),,drop=FALSE],2,diff)
      s[i,j,] <- apply(q[c(notj[i],j),,drop=FALSE],2,sum)
    }
  }

  # Beta difference mapping
  if (!all(c("DS","DD") %in% dimnames(pars)[[2]]))
    d <- (1+as.vector(d))/2 else {
...
advantage_parameters <- function(pars,SV,nr,pname="v")  {
  na <- nr-1                     # accumulators within responses
  nt <- nrow(pars)/nr            # trials
  q <- matrix(SV,nrow=nr)
  ...
  # expand pars
  rpars <- apply(pars[,dimnames(pars)[[2]]!=pname],2,rep,each=na)
  v <- (rep(pars[,"AS"],each=na)*as.vector(s) +
       rep(pars[,"AD"],each=na)*as.vector(d))/2
  rpars <- cbind(v,rpars)
  dimnames(rpars)[[2]][1] <- pname
  rpars
}
