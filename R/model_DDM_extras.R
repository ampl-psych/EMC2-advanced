  ...
  list(
    ...
    rfun=function(lR=NULL,pars) {
     ok <- !( abs(pars[,"v"])> 20 | pars[,"a"]> 10 | pars[,"a"] < 0 | pars[,"sv"] > 10 |
      ...
      if (pars[1,"sv"] !=0) attr(pars,"ok") <- attr(pars,"ok") & pars[,"sv"] > .001
      if (pars[1,"SZ"] !=0) attr(pars,"ok") <- attr(pars,"ok") & pars[,"SZ"] > .001
      if (is.null(lR)) ok else {
        out <- rDDM(lR,pars,precision=2.5,ok)
    ...
    pfun=function(rt,R,pars) pDDM(rt,R,pars,precision=2.5),
    log_likelihood=function(p_vector,dadm,min_ll=log(1e-10)){
      log_likelihood_ddmgng(p_vector=p_vector, dadm = dadm, min_ll = min_ll)
    }
  )
}
...
MDDM <- function(){
  list(
    ...
    Ntransform=function(x,use=NULL) {
      ...
      isprobit <- dimnames(x)[[2]] %in% c("Z","SZ","DP","pContaminant")
      x[,islog] <- exp(x[,islog])
...
ADDMnoC <- function(){
  list(
    ...
    # Transform to natural scale
    Ntransform=function(x,use=NULL) {
      lognames <- c("a","sv","t0","st0","s","AD","ssv")
      probitnames <- c("Z","SZ","DP")
      if (!is.null(use)) {
