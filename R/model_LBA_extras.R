...
rALBA <- function(lR,pars,p_types=c("v","sv","b","A","t0"),posdrift = TRUE,
...
{
  if (length(lR)==nrow(pars)) return(rLBA(lR,pars,p_types,ok)) else {
    nr <- length(levels(lR))
    na <- nr-1
    nt <- nrow(pars)/(na*nr)
    lR <- rep(lR,each=na)
    ...
    dt[dt<0] <- Inf
    bad <- apply(dt,3,function(x){all(is.infinite(x))})
    dt <- apply(dt,2:3,max)
...
#' Advantage LBA Model with no C, pbeta mapping
#'
...
BE2lbaB <- function(){
  list(
    type="MT",
    p_types=c("v" = 0,"sv" = log(1),"B" = log(.5),"A" = log(0),"t0" = log(0),r=0,
              DT1=log(.5)),
    # p_vector transform, sets sv as a scaling parameter
    transform = function(p) p,
    # Transform to natural scale
    ...
    Ttransform = function(pars,dadm) {
      ...
      attr(pars,"ok") <- (pars[,"t0"] > .05) & ((pars[,"A"] > 1e-2) | pars[,"A"] == 0) &
    ...
    # Random function for racing accumulator
    rfun=function(lR=NULL,pars) {
      ok <- (pars[,"t0"] > .05) & ((pars[,"A"] > 1e-2) | pars[,"A"] == 0) &
...
BE3lbaB <- function(){
  list(
    type="MT",
    p_types=c("v" = 0,"sv" = log(1),"B" = log(1/3),"A" = log(0),"t0" = log(0),r=0,
              DT1=log(1/3),DT2=log(1/3)),
    # p_vector transform, sets sv as a scaling parameter
    transform = function(p) p,
    # Transform to natural scale
    Ntransform=function(x) {
...
BE4lbaBP <- function(){
  list(
    type="MT",
    p_types=c("v" = 0,"sv" = log(1),"B" = log(1/4),"A" = log(0),"t0" = log(0),r=0,
              DT1=log(1/4),DT2=log(1/4),DT3=log(1/4)),
    # p_vector transform, sets sv as a scaling parameter
    transform = function(p) p,
    # Transform to natural scale
    Ntransform=function(x) {
...
#' MLBA: Missing parameterization for LBA
#'
...
#' MULBA: Missing parameterization for LBA with unbouned rates.
#'
#' LBA model accommodating missing values (truncation and censoring) and
...
MULBA <- function(){
  list(
    type="RACE",
    p_types=c("v" = 0,"sv" = log(1),"B" = log(1),"A" = log(0),"t0" = log(0),pContaminant=pnorm(0)),
    Ntransform=function(x,use=NULL) {
      # Transform to natural scale
      doexp <- !(dimnames(x)[[2]] %in% c("v","pContaminant"))
      x[,doexp] <- exp(x[,doexp])
